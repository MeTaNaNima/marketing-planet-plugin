...
add_action('admin_init', function () {
  ...
  register_setting('marketing_planet_settings_group', 'marketing_planet_faq_first_active');
  ...
  // Add actions to modify post types array before saving
  add_filter('pre_update_option_marketing_planet_tldr_post_types', function ($new_value) {
    // Ensure 'post' is always included in the array
    if (!in_array('post', (array) $new_value)) {
      $new_value[] = 'post';
    }
    return $new_value;
  });

  // Ensure default value for section title when empty
  add_filter('pre_update_option_marketing_planet_tldr_section_title', function ($new_value) {
    if (empty($new_value)) {
      $new_value = "Here is the Quick Answer:";  // Default value
    }
    return $new_value;
  });

  add_filter('pre_update_option_marketing_planet_faq_post_types', function ($new_value) {
    // Ensure 'post' is always included in the array
    if (!in_array('post', (array) $new_value)) {
      $new_value[] = 'post';
    }
...
function marketing_planet_settings_page(): void
{
  ...
  $tldr_enabled_post_types = (array) get_option('marketing_planet_tldr_post_types', ['post']);
  $faq_enabled_post_types = (array) get_option('marketing_planet_faq_post_types', ['post']);
  $faq_auto_append = get_option('marketing_planet_faq_auto_append', false);
  $faq_first_active = get_option('marketing_planet_faq_first_active', false);
  ...
  settings_errors('marketing_planet_settings_group');

?>
  <div class="wrap">
    <h1>Marketing Planet Plugin</h1>

    <h2 class="nav-tab-wrapper">
      <a href="#" class="nav-tab nav-tab-active" data-tab="tab-modules">Activate Modules</a>
      <a href="#" class="nav-tab" data-tab="tab-tldr" style="<?php (in_array('tldr-field', $active_modules) ? '' : 'display:none;'); ?>">TL;DR Settings</a>
      <a href="#" class="nav-tab" data-tab="tab-faq" style="<?php (in_array('faq-repeater', $active_modules) ? '' : 'display:none;'); ?>">FAQ Settings</a>
      <a href="#" class="nav-tab" data-tab="tab-sitemap-exclude" style="<?php (in_array('sitemap-exclude-post-types', $active_modules) ? '' : 'display:none;'); ?>">Yoast Sitemap Control</a>
      <a href="#" class="nav-tab" data-tab="tab-styles">Add/Edit Styles</a>
      <a href="#" class="nav-tab" data-tab="tab-shortcodes">Shortcode Helper</a>
      <a href="#" class="nav-tab" data-tab="tab-about">About</a>
    </h2>
    ...
    <form method="post" action="options.php">
      ...
      <div class="tab-section" id="tab-modules">
        ...
        <hr>
        <p>
          <?php echo get_option('marketing_planet_tldr_section_title', 'Here is the Quick Answer:'); ?>
        </p>
        <button id="debug-info" class="debug-info-button button-secondary">Show Debug Info</button>
        <div class="debug-info-container" style="display:none;">
          <pre>
            All Modules: <?php print_r($module_folders); ?>
          </pre>
          <pre>
            Active Modules: <?php print_r($active_modules); ?>
          </pre>
        </div>
      </div>
      ...
      ?>
        <div class="tab-section" id="tab-tldr" style="display:none;">
          <h2>TL;DR Settings</h2>
          <table class="form-table">
            <tr>
              ...
              <td>
                <div class="post-type-wrapper">
                  ...
                  foreach ($post_types as $type) {
                    // Check if it's a blog post type and make it selected by default
                    $selected = in_array($type->name, $tldr_enabled_post_types) || $type->name === 'post' ? 'checked' : '';
                    echo "<label><input " . ($type->name === 'post' ? 'disabled' : '') . " type='checkbox' name='marketing_planet_tldr_post_types[]' value='{$type->name}' {$selected}> {$type->label}</label>";
                  }
                  ?>
                </div>
              </td>
            ...
            <tr>
              ...
              <td>
                <input type="text" name="marketing_planet_tldr_section_title" value="<?php echo empty($tldr_section_title) ? "Here is the Quick Answer:" : esc_attr($tldr_section_title); ?>" style="min-width: 250px;">
                <p class="description">Leave empty to use the default: "Here is the Quick Answer:"</p>
              </td>
            </tr>
          </table>
        </div>
      ...
      ?>
        <div class="tab-section" id="tab-faq" style="display:none;">
          <h2>FAQ Settings</h2>
          <table class="form-table">
            <tr>
              <th scope="row">Post Types</th>
              <td>
                <div class="post-type-wrapper">
                  ...
                  foreach ($post_types as $type) {
                    // Check if it's a blog post type and make it selected by default
                    $selected = in_array($type->name, $faq_enabled_post_types) || $type->name === 'post' ? 'checked' : '';
                    echo "<label><input " . ($type->name === 'post' ? 'disabled' : '') . " type='checkbox' name='marketing_planet_faq_post_types[]' value='{$type->name}' {$selected}> {$type->label}</label>";
                  }
                  ?>
                </div>
              </td>
            ...
            <tr>
              ...
              <td>
                <label><input type="checkbox" name="marketing_planet_faq_auto_append" value="1" <?php echo ($faq_auto_append ? ' checked' : ''); ?>> Automatically append FAQ after post content</label>
                <p>Manual usage: <code id="mp_faq_shortcode" title="Click on the shortcode to copy!" style="cursor:pointer;" onclick="copyMPShortcode()">[mp_faqs]</code> <span id="mp_faq_copy_feedback" style="display:none; color:green; margin-left:10px;">Copied!</span></p>
              </td>
            </tr>
            <tr>
              <th scope="row">First Active</th>
              <td>
                <label><input type="checkbox" name="marketing_planet_faq_first_active" value="1" <?php echo ($faq_first_active ? ' checked' : ''); ?>> Make the First FAQ Active</label>
              </td>
            </tr>
          </table>
        </div>
      ...
      ?>
      <div class="tab-section" id="tab-styles" style="display:none;">
        <h3>Custom CSS</h3>
        <p>Click to init Code editor.</p>
        <textarea id="mp-custom-css" name="marketing_planet_custom_css" rows="4" style="width:100%; font-family: monospace;"><?php echo esc_textarea(get_option('marketing_planet_custom_css', '')); ?></textarea>

        <?php
        if (in_array('tldr-field', $active_modules)) {
        ?>
          <h3>TL;DR Box Styles</h3>
          <table class="form-table">
            <?php
        ...
        ?>
          <p><em>Activate the TL;DR module to configure its styles here.</em></p>
        <?php
        }
        ?>
      </div>
      ...
      ?>
        <div class="tab-section" id="tab-sitemap-exclude" style="display:none;">
          <?php
      ...
      <div class="tab-section" id="tab-shortcodes" style="display:none;">
        ...
        <table class="widefat striped">
          ...
          <tbody>
            ...
            ?>
            <tr>
              <td><code class="mp-copyable-shortcode" title="Click to copy" style="cursor:pointer;">[mp_date]</code></td>
              <td>Displays the current date using the default format (Y-m-d).</td>
              <td><code>[mp_date]</code> → 2025-06-08</td>
            </tr>
            <tr>
              <td><code class="mp-copyable-shortcode" title="Click to copy" style="cursor:pointer;">[mp_date format="F j, Y"]</code></td>
              <td>Displays a custom formatted date.</td>
              <td><code>[mp_date format="F j, Y"]</code> → June 8, 2025</td>
            </tr>
            <tr>
              <td><code class="mp-copyable-shortcode" title="Click to copy" style="cursor:pointer;">[mp_date format="H:i" timezone="Europe/Berlin"]</code></td>
              <td>Uses a specific timezone for the date output.</td>
              <td><code>[mp_date format="H:i" timezone="Europe/Berlin"]</code> → 16:42</td>
            </tr>
            ...
            // Example for FAQs
            <tr>
              <td><code class="mp-copyable-shortcode" title="Click to copy" style="cursor:pointer;">[mp_faqs]</code></td>
              <td>Displays FAQs manually on the page.</td>
              <td><code class="mp-copyable-shortcode" title="Click to copy" style="cursor:pointer;">[mp_faqs]</code></td>
            </tr>

          </tbody>
        </table>
        <span id="mp_shortcode_copy_feedback" style="display:none; color:green; margin-top:10px;">Copied!</span>
      </div>


      <div class="tab-section" id="tab-about" style="display:none;">
        <p><strong>Marketing Planet Plugin</strong> is a modular toolset for reusable functionality, SEO, content enhancements, and more — built for performance-oriented WordPress development.</p>
        <p>Developed by <a href="https://marketingplanet.agency/" target="_blank">Marketing Planet Agency</a></p>
        ...
        <table class="form-table">
          ...
          <tr>
            ...
            <td>
              <input type="password" name="marketing_planet_github_token" value="<?php echo esc_attr($github_token); ?>" class="regular-text" style="min-width: 250px;">
              <p class="description">Enter your GitHub personal access token to enable plugin updates from private repo!</p>
            </td>
          </tr>
        </table>
      ...
      wp_nonce_field('marketing_planet_settings', 'marketing_planet_nonce');
